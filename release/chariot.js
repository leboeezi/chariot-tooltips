    ...
    var Overlay = function () {
      ...
      _createClass(Overlay, [{
        ...
        value: function _createOverlay() {
          var $overlay = $("<div class='chariot-overlay'><div class='chariot-overlay-child'>H</div></div>");
        ...
        value: function _waitForElement(selectorName, numAttempts, resolve, reject) {
          ...
          if (element.length == 0) {
            ...
            for (let i = 0; i < mainElement.length; i++){
              this._elementMap[selectorName].element[i].classList.add("chariot-mark");
              // this._elementMap[selectorName].element[i].style.border = "3px solid rgba(255,255,64)";
            }
            resolve();
    ...
    var Tooltip = function () {
      ...
      _createClass(Tooltip, [{
        ...
        value: function _iconMarkup() {
          ...
          return '<div class="chariot-tooltip-icon"><div class="'+ this.iconUrl + '"/></div>';
        }
      }, {
        key: '_subtextMarkup',
        ...
        value: function _positionTooltip($tooltip) {
          ...
          var mainRect = mainElement[0].getBoundingClientRect();
          var anchorRect = $anchorElement[0].getBoundingClientRect();
          var right = null;
          if(($(window).innerWidth() - anchorRect.right < 145) && (this.position == 'bottom') || (this.position == 'top')){
            $($tooltip).children()[0].classList.add('padded-arrow');
            if(anchorRect.width < ($(window).innerWidth() / 2)){
              right = $(window).innerWidth() - anchorRect.right;
              // left = left - 87;
            }
          }
          //if the selected element is 50px or less from the bottom of the screen
          if(($(window).innerHeight() - $anchorElement[0].getBoundingClientRect().bottom) <= 50){
            //scroll the element into view
            $anchorElement[0].scrollIntoView()
            //and then get a new top position
            top = _style2.default.calculateTop($tooltip, $anchorElement, this.yOffsetTooltip, this.position, this.arrowLength + this.borderTopWidth + this.borderBottomWidth);
          }else if($anchorElement[0].getBoundingClientRect().top < 50){
            //else if the selected element is 50px or less from top of the screen
            $anchorElement[0].scrollIntoView()
            //we scroll it into view
            top = _style2.default.calculateTop($tooltip, $anchorElement, this.yOffsetTooltip, this.position, this.arrowLength + this.borderTopWidth + this.borderBottomWidth);
            //and then get the new top position
          }
          if(right == null){
            var tooltipStyles = {
              top: top,
        ...
        */
        ...
        value: function _animateTooltipDisappear($tooltip) {
          for (let i = 0; i < mainElement.length; i++){
            mainElement[i].classList.remove("chariot-mark");
            // mainElement[i].style.border = "none";
          }
          var _this4 = this;
    ...
    var Tutorial = function () {
      ...
      _createClass(Tutorial, [{
        ...
        value: function next(step) {
          ...
          mainElement[0].classList.remove("chariot-mark");
          // mainElement[0].style.border = "none";
          nextStep.render();
        }
